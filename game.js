// game.js - 孖寶兄弟 1985 風格 Online 一關單人版 (踩怪BUG已修復！)
...
function loop() {
    ...
    if (gameOver && keys[' ']) {
        ...
        // 重置所有怪物位置與狀態
        const enemyPositions = [600,1100,1600,2100,2500];
        const enemyVxs = [-2,2,-2,2,-2];
        const enemyTimers = [0,100,200,0,150];
        enemies.forEach((e, i) => {
            e.x = enemyPositions[i];
            e.vx = enemyVxs[i];
            e.timer = enemyTimers[i];
        });
        gameOver = false;
        won = false;
        camera.x = 0;
